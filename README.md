...
`<Space>b` buffers, `<Space>:` command palette, `<Space>h` help, `<Space>?`
every key there is, `<Space>a` the code tasks (`aa` code action, `ar` rename,
`af` format, `aF` organize imports, `ai` hover, `ax` run the code lens on this
line, `ad` / `aD` diagnostics here / everywhere), `<Space>g` where to go (`gd` /
`gD` definition / declaration, `gi` implementation, `gt` type, `gr` references,
`gs` / `gS` symbols here / everywhere), `<Space>t` the toggles (`tt` the file
tree, `` t` `` the terminal, `tw` wrapping long lines in this window, `th` and
`ti` dotfiles and ignored files in both the tree and the finders), `Ctrl+Del`
